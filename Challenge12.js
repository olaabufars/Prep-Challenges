...
const cvsFiltration = (arr) => {
    let ExpeTec = arr.filter(exp =>(exp.yearsOfExperience >= 4 & exp.tech == "JS"))
    function checkName(full){
        if(full.LastName != null){
            full.fullName = full.firstName +" "+full.LastName;
            delete full.firstName;
            delete full.LastName;
        }else{
            full.fullName = full.firstName ;
            delete full.firstName;
            delete full.LastName;
        }
    }checkName(ExpeTec)
...
const vowelsFiltration = (arr) => {
    var fillter1=arr.filter((allY) => !['car', 'boy','building'].includes(allY));
    return fillter1
    }
